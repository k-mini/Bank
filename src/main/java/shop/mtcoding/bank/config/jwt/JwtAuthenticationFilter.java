...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    // Post : /login -> /api/login setFilterProcessesUrl("/api/login")
    @Override
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // 강제 로그인 => 인증 관련 토큰 (JWT 토큰과 무관함)
            UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
                    loginReqDto.getUsername(), loginReqDto.getPassword());

            // UserDetailsService의 loadUserByUsername 호출
            // JWT를 쓴다 하더라도, 컨트롤러 진입을 하면 시큐리티의 권한체크, 인증체크의 도움을 받을 수 있게
            // 세션을 만든다. 이 세션의 유효기간은 request하고, response하면 끝!!
