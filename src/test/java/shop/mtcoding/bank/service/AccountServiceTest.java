...
public class AccountServiceTest extends DummyObject {
    ...
    public void 계좌입금_test() throws Exception {
        ...
        System.out.println("테스트 : 계좌1쪽 잔액" + ssarAccount1.getBalance());
        System.out.println("테스트 : 계좌2쪽 잔액" + ssarAccount2.getBalance());
    ...
    // 서비스를 테스트 하고 싶다면, 내가 지금 무엇을 여기서 테스트해야할지 명확히 구분 (책임 분리)
    // DTO를 만드는 책임 -> 서비스에 있지만!! (서비스에서 DTO검증 안할래!! - Controller 테스트 해볼 것이니까)
    // DB관련된 것도 -> 서비스 것이 아니야.. 볼필요없어
    // DB관련된 것을 조회했을 때, 그 값을 통해서 어떤 비즈니스 로직이 흘러가는 것이 있으면 -> stub으로 정의해서 테스트 해보면 된다.
